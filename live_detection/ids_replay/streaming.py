...
class LiveEngine:
    """Phát flow LIVE: liên tục quét drop_dir (CSV từ WSL cfm) → broadcast qua WebSocket.

    Song song ReplayEngine, dùng chung ConnectionManager + format message ('flow').
    Khác biệt: nguồn vô hạn theo thời gian thực, không có tổng số n, truth rỗng.
    """

    def __init__(self, source, manager: ConnectionManager, poll_interval: float = 1.0):
        self.source = source
        self.manager = manager
    ...
    async def start(self) -> bool:
        self.stop()
        await in_thread(self.source.reset)      # dọn CSV cũ, bắt đầu phiên sạch
        self._idx = 0
        self.running = True
        await self.manager.broadcast({"type": "reset"})
        await self.manager.broadcast({"type": "live_start", "label": "LIVE (WSL capture)"})
    ...
    async def _loop(self):
        try:
            while self.running:
                events = await in_thread(self.source.poll_new)
                if not events:
                    await asyncio.sleep(self.poll_interval)
                    continue
                for e in events:
                    await self.manager.broadcast({"type": "flow", "idx": self._idx,
                                                  "src": e["src"], "dst": e["dst"],
                                                  "dst_port": e["dst_port"], "truth": e["true"],
                                                  "pred": e["pred"], "conf": e["conf"]})
                    self._idx += 1
                    await asyncio.sleep(1.0 / BASE_RATE)   # nhịp hiển thị mượt
                await self.manager.broadcast({"type": "live_batch", "total": self._idx})
        except asyncio.CancelledError:
            pass
