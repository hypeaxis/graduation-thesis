...
def load_chunk_data(train_csv, val_csv, batch_size=256):
    print("Loading Train Stage 2...")
    df_train = pd.read_csv(train_csv)
    print("Loading Val Full (Filtering for Stage 2 evaluation)...")
    df_val = pd.read_csv(val_csv)
    
    mapping = {
        'BENIGN': 'Benign', 'Web Attack.*': 'Web Attack',
        'Bot': 'Rare Attacks', 'Infiltration': 'Rare Attacks', 'Heartbleed': 'Rare Attacks'
    }
    df_val['Label_Stage2'] = df_val['Label'].replace({'Web Attack.*': 'Web Attack'}, regex=True).map(mapping)
...
def main():
    TRAIN_CSV = 'processed_data/cic_train_stage2.csv'
    VAL_CSV = 'processed_data/cic_test_full.csv'
    SAVE_DIR = 'models/v4_cascade/stage2'
    ...
    for cls_name in ['Web Attack', 'Rare Attacks']:
    ...
    print("Hoàn tất Stage 2 Training!")
