...
def load_chunk_data(train_csv, val_csv, batch_size=256):
    ...
    val_usecols = [c for c in STAGE2_FEATURES if c not in ['Flow_Bytes_Ratio', 'Flow_Pkts_Ratio']] + ['Label', 'Total_Length_of_Fwd_Packets', 'Total_Length_of_Bwd_Packets', 'Total_Fwd_Packets', 'Total_Backward_Packets']
    df_val = pd.read_csv(val_csv, usecols=val_usecols)
    df_val['Flow_Bytes_Ratio'] = df_val['Total_Length_of_Fwd_Packets'] / (df_val['Total_Length_of_Bwd_Packets'].replace(0, 1))
    df_val['Flow_Pkts_Ratio'] = df_val['Total_Fwd_Packets'] / (df_val['Total_Backward_Packets'].replace(0, 1))
    df_val['Flow_Bytes_Ratio'] = df_val['Flow_Bytes_Ratio'].replace([np.inf, -np.inf], 0).fillna(0)
    df_val['Flow_Pkts_Ratio'] = df_val['Flow_Pkts_Ratio'].replace([np.inf, -np.inf], 0).fillna(0)
