...
\input{lstlisting} % Cấu hình định dạng hiển thị code nguồn
...
% ===================================================
\usepackage{hyperref}
\hypersetup{pdfborder = {0 0 0}, unicode=true}
...
% Cải thiện ngắt dòng - tránh tràn lề phải
\tolerance=1000                    % Cho phép giãn dòng linh hoạt hơn (mặc định 200)
\emergencystretch=2em              % Thêm khoảng giãn khẩn cấp khi không thể ngắt dòng
\hfuzz=3pt                         % Bỏ qua tràn lề nhỏ hơn 3pt (không nhìn thấy được)
...
\chapter{CẤU HÌNH MÔI TRƯỜNG THỰC NGHIỆM}
\subfile{Chuong/Phu_luc_A}

\newpage
\chapter{THÔNG SỐ MÔ HÌNH VÀ KẾT QUẢ CHI TIẾT}
