...
\textbf{Giai đoạn 3} giải quyết covariate shift: mô hình CIC chỉ đạt 21,93\% Accuracy khi áp dụng trực tiếp lên Testbed WSL (MCC = $-0{,}015$), trong khi Re-fit Scaler đơn thuần còn làm tình trạng xấu hơn (6,87\%). Sau chẩn đoán định lượng, giải pháp là thu thập dữ liệu Testbed đặc thù và tinh chỉnh mô hình bằng fine-tuning trên nền backbone của Giai đoạn 2, kết hợp Model Surgery mở rộng không gian đặc trưng. Mô hình FT-IDS giải quyết hai bài toán đặc thù WSL: PortScan không phân tách trong NAT (dữ liệu surrogate từ CIC Friday hardware) và BruteForce thiếu diversity (mixed hydra + CIC Patator). Kết quả: Macro F1 91,66\% trên val set đa dạng miền.

Hai tầng được tích hợp thành một hệ thống lai ghép: Snort ở tầng gói (rule-based) bù đắp cho FT-Transformer ở tầng flow (ML-based) để đạt độ bao phủ tấn công rộng hơn bất kỳ hướng tiếp cận đơn lẻ nào. Hệ thống được hiện thực hóa và đánh giá ở dạng phát hiện theo phát lại (replay-based live detection); đường xử lý thời gian thực hoàn toàn được để lại như một hướng phát triển.
...
\begin{enumerate}
    ...
    \item \textbf{Hard Negative Mining 2 vòng cho lớp tấn công hiếm:} Thu thập mẫu bị phân loại sai sau mỗi vòng huấn luyện và nhân bản 10 lần bổ sung vào tập huấn luyện tiếp theo. So sánh trực tiếp trên lớp Botnet, chiến lược này đạt F1 = 0,6512 so với 0,5103 của phương án nhân trọng số lớp $\times 10$ --- cho thấy việc tập trung tín hiệu học vào vùng biên quyết định hiệu quả hơn tăng cường đều tay trên toàn lớp.

    \item \textbf{Chẩn đoán covariate shift định lượng và xây dựng Testbed thực:} Định lượng mức độ covariate shift: direct transfer Accuracy 21,93\% (MCC $-0{,}015$); Re-fit Scaler làm tình trạng xấu hơn (6,87\%). Thu thập tập dữ liệu Combined (111.825 flows, 5 lớp) trong môi trường mạng LAN thực; xác định và giải quyết PortScan không phân tách trong NAT bằng dữ liệu surrogate; đánh giá robustness với val set đa dạng miền. Mô hình FT-IDS đạt Macro F1 91,66\%.

    \item \textbf{Hệ thống Hybrid IDS phát hiện theo phát lại trên phần cứng phổ thông:} Hệ thống tích hợp Snort 3 và FT-IDS trên WSL2 Ubuntu, không yêu cầu GPU, được hiện thực hóa ở dạng phát hiện theo phát lại với giao diện thời gian thực. Thực nghiệm xác nhận tính bổ sung hai tầng: DoS slowhttptest chỉ phát hiện được bởi ML, XSS payload ngắn chỉ phát hiện được bởi Snort. Một luật hậu xử lý kiểu dấu hiệu (đếm cổng đích theo nguồn) bù cho đặc trưng quét cổng mà mô hình thiếu, nâng PortScan F1 từ 0,000 lên 0,996 mà không cần huấn luyện lại. Đường xử lý thời gian thực hoàn toàn được đề xuất như một hướng phát triển.
\end{enumerate}
