...
if __name__ == '__main__':
    ...
    train_in = base_dir / 'data' / 'KDDTrain+.txt'
    test_in = base_dir / 'data' / 'KDDTest+.txt'
    out_dir = base_dir / 'data' / 'processed'
    artifacts_dir = base_dir / 'models' / 'artifacts_preprocess'

    out_dir.mkdir(parents=True, exist_ok=True)
    
    if not train_in.exists() or not test_in.exists():
        print(f"Error: {train_in} or {test_in} not found. Please download NSL-KDD dataset first.")
        exit(1)

    print("Running basic preprocessing (v2 - 122 features)...")
    fit_train_preprocessor(str(train_in), str(out_dir / 'cleaned5Grouped_v2_KddTrain+.csv'), str(artifacts_dir), data_version='v2')
    transform_test_with_preprocessor(str(test_in), str(out_dir / 'cleaned5Grouped_v2_KddTest+.csv'), str(artifacts_dir), data_version='v2')
    run_schema_sanity_checks(str(out_dir / 'cleaned5Grouped_v2_KddTrain+.csv'), str(out_dir / 'cleaned5Grouped_v2_KddTest+.csv'))
