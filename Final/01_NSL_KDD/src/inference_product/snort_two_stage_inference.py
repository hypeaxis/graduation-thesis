...
# File này nằm ở 01_NSL_KDD/src/inference_product/.
# Trọng số/JSON nằm ở 01_NSL_KDD/models/ (PKG_ROOT/models), không phải dưới src/.
HERE = Path(__file__).resolve().parent          # inference_product/
PKG_ROOT = HERE.parents[1]                       # -> 01_NSL_KDD/
if str(HERE) not in sys.path:                    # để import module cùng thư mục
    sys.path.insert(0, str(HERE))

from phase2_ft_transformer import FTTransformer
from snort_preprocess_122 import (
...
DEFAULT_FT_CHECKPOINT = PKG_ROOT / 'models' / 'best_model.pt'
DEFAULT_AE_CHECKPOINT = PKG_ROOT / 'models' / 'autoencoder_v2_best.h5'
DEFAULT_INFERENCE_CONFIG = PKG_ROOT / 'models' / 'inference_config.json'
DEFAULT_SCALER = PKG_ROOT / 'models' / 'scaler.pkl'
DEFAULT_FEATURE_COLUMNS = PKG_ROOT / 'models' / 'feature_columns.json'
DEFAULT_SNORT_ALERT = PKG_ROOT / 'log' / 'alert.csv'
DEFAULT_SNORT_FEATURES = PKG_ROOT / 'log' / 'snort_features_122.csv'
DEFAULT_OUTPUT = PKG_ROOT / 'log' / 'snort_ft_transformer_predictions.csv'
