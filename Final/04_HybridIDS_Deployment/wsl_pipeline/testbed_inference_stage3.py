...
def main():
    print("="*60)
    print("TESTBED INFERENCE: ĐÁNH GIÁ MÔ HÌNH STAGE 3 (80 FEATURES)")
    print("="*60)
    ...
    try:
        ...
        print(f"Lỗi: Không tìm thấy file Raw. {e}")
        return

    df = df[df['Label'].isin(['Malicious', 'Benign'])].copy()
    
    print("[*] Mapping tên cột và sinh 80 đặc trưng...")
    ...
    print("[*] Đang nạp Scaler và Model FT-Transformer (Stage 3)...")
    MODEL_DIR_CIC = os.path.abspath(os.path.join(WORKSPACE_DIR, '../models/v4_cascade/stage1'))
    scaler_old = joblib.load(os.path.join(MODEL_DIR_CIC, 'scaler_stage1.pkl'))
    encoder1 = joblib.load(os.path.join(MODEL_DIR_CIC, 'encoder_stage1.pkl'))

    # Prepare scaler (fit Standard scaler on new features just for this inference script)
    X_old = X_raw[:, :77]
    X_new = X_raw[:, 77:]
    X_scaled_old = scaler_old.transform(X_old)
    scaler_new = StandardScaler()
    X_scaled_new = scaler_new.fit_transform(X_new)
    X_scaled = np.hstack((X_scaled_old, X_scaled_new))

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model1 = FTTransformer(num_features=80, num_classes=len(encoder1.classes_), 
                           d_model=128, num_layers=4, num_heads=8, dropout=0.2, drop_path_rate=0.1).to(device)
    
    model_path = os.path.abspath(os.path.join(WORKSPACE_DIR, '../../Domain_Adaptation_Workspace/models/finetuned_stage3_feature_eng.pt'))
    model1.load_state_dict(torch.load(model_path, map_location=device))
    model1.eval()

    print("[*] Áp dụng chuẩn hóa và chạy dự đoán...")
    ...
    print("\n" + "="*60)
    print("BÁO CÁO PHÂN LỚP TRÊN TESTBED (STAGE 3 MODEL)")
    print("="*60)
