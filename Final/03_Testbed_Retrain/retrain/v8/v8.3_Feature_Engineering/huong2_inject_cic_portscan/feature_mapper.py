"""
feature_mapper.py — Hướng 2 V8.3
Chuyển đổi CIC-IDS-2017 raw CSV → 81 features tương thích testbed.

Công thức custom features lấy trực tiếp từ dataset_builder_v2.py::extract_custom_features()
và dataset_builder_v2.py::create_port_categories().
...
# Chỉ map các cột có trong EXPECTED_FEATURES_81
CIC_TO_TESTBED = {
    ...
    ' Idle Min':                   'Idle_Min',
    ' Label':                      'Label',
    ' Destination Port':           '_dst_port_tmp',  # dùng cho Port_Is_* rồi drop
}
...
def compute_custom_features(df):
    ...
    # Không thể tính từ CIC (cần Src IP + sliding window theo timestamp)
    df['Custom_PortScan_Intensity'] = 0.0

    return df


def main():
    print(f"[*] Đọc CIC-IDS-2017 PortScan file...")
    ...
    print(f"\n[*] Label distribution ban đầu:")
    print(df['Label'].value_counts().to_string())

    # Chỉ giữ PortScan
    df = df[df['Label'] == 'PortScan'].reset_index(drop=True)
    print(f"\n[*] Sau khi lọc PortScan: {len(df):,} rows")

    # Tính Port_Is_* (cần _dst_port_tmp)
    if '_dst_port_tmp' in df.columns:
        df = compute_port_categories(df)
        df.drop(columns=['_dst_port_tmp'], inplace=True)
    else:
    ...
    # Tính Custom_* features
    df = compute_custom_features(df)

    # Xử lý inf/NaN
    df.replace([np.inf, -np.inf], np.nan, inplace=True)
    nan_before = df.isnull().sum().sum()
    df.dropna(subset=EXPECTED_FEATURES_81, inplace=True)
    print(f"[*] Đã xoá {nan_before} NaN/Inf cells → còn {len(df):,} rows")

    # Kiểm tra đủ 81 features chưa
    missing = [f for f in EXPECTED_FEATURES_81 if f not in df.columns]
    if missing:
        print(f"\n[!] THIẾU {len(missing)} features: {missing}")
        for f in missing:
            df[f] = 0.0
        print("    → Đã điền 0.0 cho các feature thiếu")
    else:
        print(f"[+] Đủ 81 features ✓")

    # Lưu
    output_cols = EXPECTED_FEATURES_81 + ['Label']
    df[output_cols].to_csv(OUTPUT_PATH, index=False)
    print(f"\n[+] Saved: {OUTPUT_PATH} ({len(df):,} rows, {len(output_cols)} columns)")

    # Quick verify
    print("\n=== Verify ===")
    print(f"Flow_Duration mean: {df['Flow_Duration'].mean():.0f} μs")
    print(f"RST_Flag_Count mean: {df['RST_Flag_Count'].mean():.2f}")
    print(f"Custom_PortScan_Intensity: {df['Custom_PortScan_Intensity'].unique()} (kỳ vọng: [0.0])")
