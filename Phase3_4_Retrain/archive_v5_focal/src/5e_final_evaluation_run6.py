...
def load_test_data(filepath, encoder):
...
def main():
    print("=========================================================")
    print("5E. ĐÁNH GIÁ CHÍNH THỨC TRÊN TẬP TEST (RUN 6)")
    print("=========================================================")
    ...
    # 1. LOAD ENCODER & THRESHOLD
    encoder = joblib.load(ENCODER_PATH)
    benign_idx = encoder.transform(['Benign'])[0]
    
    with open(THRESHOLD_PATH, 'r') as f:
        threshold_config = json.load(f)
    best_threshold = threshold_config['optimal_benign_threshold']
    print(f"[*] Đã nạp Optimal Benign Threshold = {best_threshold}")
    
    # 2. LOAD DATA
    print(f"[*] Nạp Test Data: {TEST_DATA_PATH}...")
    X_test, y_test = load_test_data(TEST_DATA_PATH, encoder)
    y_test_encoded = encoder.transform(y_test)
    print(f"    Tổng mẫu hợp lệ trong run6: {len(X_test)}")
    ...
    # 5. INFERENCE
    print("[*] Thực hiện dự đoán trên Test Set...")
    ...
    # 6. ÁP DỤNG THRESHOLD CALIBRATION
    non_benign_probs = all_probs.copy()
    non_benign_probs[:, benign_idx] = -1.0
    non_benign_preds = np.argmax(non_benign_probs, axis=1)
    
    final_preds_encoded = np.where(all_probs[:, benign_idx] >= best_threshold, benign_idx, non_benign_preds)
    final_preds_labels = encoder.inverse_transform(final_preds_encoded)
    
    # 7. BÁO CÁO KẾT QUẢ
    print("\n=========================================================")
    print("BÁO CÁO KẾT QUẢ (CON SỐ CHÍNH THỨC DÙNG CHO LUẬN VĂN)")
    print("=========================================================")
