...
def main():
    ...
    # --- Load V8.4 model ---
    print(f"\n[*] Nạp V8.4 model: {BASE_MODEL_PATH}")
    ...
    for epoch in range(EPOCHS):
        ...
        bf  = report.get('Brute Force', {})
        wa  = report.get('Web Attack', {})
        ps  = report.get('PortScan', {})
        bf_f1 = bf.get('f1-score', 0)
        bf_p  = bf.get('precision', 0)
        bf_r  = bf.get('recall', 0)
        wa_f1 = wa.get('f1-score', 0)
        ps_f1 = ps.get('f1-score', 0)

        print(f"  Epoch {epoch+1:02d}/{EPOCHS} | Loss: {total_loss/len(loader):.4f} | "
              f"B-Acc: {b_acc:.4f} | Macro F1: {macro_f1:.4f} | "
              f"BF: {bf_f1:.4f}(P{bf_p:.2f}/R{bf_r:.2f}) | "
              f"WA: {wa_f1:.4f} | PS: {ps_f1:.4f}")
        ...
        if macro_f1 > best_macro_f1:
            ...
            if no_improve_cnt >= EARLY_STOP_PAT:
                print(f"\n[!] Early stop tại epoch {epoch+1} "
                      f"(không cải thiện {EARLY_STOP_PAT} epoch liên tiếp)")
                break
