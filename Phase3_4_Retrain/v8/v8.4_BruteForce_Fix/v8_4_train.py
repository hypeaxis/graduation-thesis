"""
v8_4_train.py — V8.4: Fix Brute Force bằng CIC FTP+SSH Patator injection
Chiến lược:
  - Base:    v8_3_huong2_model.pt (Macro F1 88.7% — tốt nhất hiện tại)
  - Data:    Combined_V8_4.csv (Run10 BF thay bằng CIC BF 4,999 flows)
  - lr:      2e-5 (thấp hơn H2 để fine-tune nhẹ, tránh catastrophic forgetting)
  - Epochs:  25
  - Weights: sqrt-balanced, capped [0.5, 2.0] — cân bằng precision/recall BF
"""
...
# PATHS
# ============================================================================
DATA_PATH        = os.path.join(WORKSPACE_DIR, 'Combined_V8_4.csv')
BASE_MODEL_PATH  = os.path.abspath(os.path.join(
    WORKSPACE_DIR, '../v8.3_Feature_Engineering/huong2_inject_cic_portscan/v8_3_huong2_model.pt'))
...
MODEL_SAVE_PATH   = os.path.join(WORKSPACE_DIR, 'v8_4_model.pt')
SCALER_SAVE_PATH  = os.path.join(WORKSPACE_DIR, 'v8_4_scaler.pkl')
ENCODER_SAVE_PATH = os.path.join(WORKSPACE_DIR, 'v8_4_encoder.pkl')
...
# ============================================================================
# FEATURES — 80 features (khớp với v7_model.pt / H2 model)
# ============================================================================
...
def main():
    print("=" * 60)
    print("V8.4: Fix Brute Force — CIC FTP+SSH Patator Injection")
    print("=" * 60)
    ...
    # --- Load H2 model làm base ---
    print(f"\n[*] Nạp H2 model: {BASE_MODEL_PATH}")
    ...
    print("    H2 model loaded (Macro F1 88.7%) — full unfreeze.")

    # --- Class weights: sqrt-balanced, capped [0.5, 2.0] ---
    # H2 uniform (1.0) → BF recall 54% (quá thụ động)
    # H1 sqrt-balanced (1.57) → BF recall 98% nhưng precision 34% (quá nhạy)
    # V8.4: sqrt-balanced NHƯNG clip max=2.0 → BF weight ~1.5–1.6, kiểm soát được
    ...
    print(f"[*] Class weights (sqrt-balanced, clip[0.5,2.0], normalized):")
    ...
    # --- Training ---
    print(f"\n[*] Bắt đầu training ({EPOCHS} epochs, lr={LR})...")
    best_macro_f1 = -1
    ...
    for epoch in range(EPOCHS):
        ...
        bf_f1  = report.get('Brute Force', {}).get('f1-score', 0)
        bf_p   = report.get('Brute Force', {}).get('precision', 0)
        ...
        print(f"  Epoch {epoch+1:02d}/{EPOCHS} | Loss: {total_loss/len(loader):.4f} | "
              f"B-Acc: {b_acc:.4f} | Macro F1: {macro_f1:.4f} | "
              f"BF F1: {bf_f1:.4f} (P:{bf_p:.2f}/R:{bf_r:.2f}) | PS F1: {ps_f1:.4f}")
