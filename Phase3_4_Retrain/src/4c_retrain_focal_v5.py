...
def main():
    print("="*60)
    print("GIAI ĐOẠN 4C: RETRAIN V5 (FOCAL LOSS + 5 CLASSES)")
    print("="*60)
    ...
    SCALER_SAVE_PATH = '../models/v5_hybrid_pipeline.pkl'
    MODEL_SAVE_PATH = '../models/v5_focal_model.pt'
    ENCODER_SAVE_PATH = '../models/v5_encoder.pkl'
    
    os.makedirs('../models', exist_ok=True)
    
    # 1. TẠO ENCODER MỚI (5 CLASSES CỦA TESTBED)
    ...
    print(f"    Tổng mẫu hợp lệ trong run5: {len(X_tb)}")
    ...
    # 4. MODEL SURGERY
    print("\n[*] Nạp Base Model và thực hiện Model Surgery (Output 5 nodes)...")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # Khởi tạo model mới với 5 classes
    model = FTTransformer(num_features=80, num_classes=5,
                          d_model=128, num_layers=4, num_heads=8, dropout=0.2)
    
    # Khởi tạo model cũ với 6 classes để nạp weight
    ...
    # Copy weights (trừ classifier layer cuối cùng)
    model_dict = model.state_dict()
    old_dict = old_model.state_dict()
    pretrained_dict = {k: v for k, v in old_dict.items() if 'classifier' not in k}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)
    
    model = model.to(device)
    
    # Freeze 77 embeddings ban đầu
    for i in range(77):
        for param in model.feature_embedding.feature_embeddings[i].parameters():
            param.requires_grad = False
            
    # Freeze 2 block transformer đầu
    for param in model.transformer_blocks[0].parameters():
        param.requires_grad = False
    for param in model.transformer_blocks[1].parameters():
        param.requires_grad = False
        
    # Train 3 feature embeddings mới
    ...
    y_encoded = torch.LongTensor(encoder.transform(y_tb_train)).to(device)
    X_tensor = torch.FloatTensor(X_train_scaled).to(device)
    
    # SỬ DỤNG TRỌNG SỐ ỔN ĐỊNH CHO FOCAL LOSS
    # Vì Focal Loss đã tự xử lý cân bằng dữ liệu thông qua (1 - pt)^gamma
    # Việc dùng class_weights quá lớn sẽ làm hỏng gradient của DoS.
    # Ta chỉ cần bơm nhẹ cho Benign để ép model cẩn thận với False Positives.
    class_weights = np.ones(5, dtype=np.float32)
    benign_idx = encoder.transform(['Benign'])[0]
    class_weights[benign_idx] = 2.5  # Phạt nặng gấp 2.5 lần nếu đoán sai Benign
    
    class_weights = torch.FloatTensor(class_weights).to(device)
    
    print(f"    Class Weights:\n      " + "\n      ".join([f"{cls}: {w:.4f}" for cls, w in zip(encoder.classes_, class_weights.cpu().numpy())]))
    
    criterion = FocalLoss(gamma=2.0, weight=class_weights, label_smoothing=0.05)
    print("    Sử dụng FocalLoss(gamma=2.0, label_smoothing=0.05) kết hợp Class Weights.")
    ...
    print("\n[+] Đã hoàn thành quá trình Retrain V5!")
