...
def main():
    print("="*60)
    print("GIAI ĐOẠN 4E: RETRAIN V7 (RUN7 DATASET + FULL UNFREEZE)")
    print("="*60)
    ...
    SCALER_SAVE_PATH = '../models/v7_hybrid_pipeline.pkl'
    MODEL_SAVE_PATH = '../models/v7_model.pt'
    ENCODER_SAVE_PATH = '../models/v7_encoder.pkl'
    ...
    # 2. LOAD DATA
    print(f"\n[*] Nạp tập dữ liệu huấn luyện: {DATA_TRAIN_PATH}...")
    X_tb, y_tb = load_and_preprocess(DATA_TRAIN_PATH, EXPECTED_FEATURES_80)
    y_encoded = encoder.transform(y_tb)
    
    # Tự động chia 1 phần nhỏ làm Validation ngay trong tập Train
    ...
    X_train_new = X_train[:, 77:]
    scaler.fit_custom_scaler(X_train_new)
    scaler.save(SCALER_SAVE_PATH)
    print(f"    Đã lưu HybridFeatureScaler tại: {SCALER_SAVE_PATH}")
    ...
    # XÓA BỎ double-dipping: Không dùng compute_class_weight vì Focal Loss đã lo việc imbalance
    cw = np.ones(len(encoder.classes_), dtype=np.float32)
    ...
    criterion = FocalLoss(gamma=2.0, weight=class_weights_tensor, label_smoothing=0.05)
    ...
    print("\n[+] Đã hoàn thành quá trình Retrain V7!")
