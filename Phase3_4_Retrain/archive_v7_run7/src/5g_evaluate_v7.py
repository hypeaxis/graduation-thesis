...
def get_predictions(model, X_scaled, device):
...
def main():
    print("=========================================================")
    print("5G. ĐÁNH GIÁ CHÍNH THỨC V7 (TRÊN TẬP RUN7 TEST)")
    print("=========================================================")

    TEST_DATA_PATH = '../data/run7_test.csv'
    SCALER_PATH = '../models/v7_hybrid_pipeline.pkl'
    MODEL_PATH = '../models/v7_model.pt'
    ENCODER_PATH = '../models/v7_encoder.pkl'
    
    encoder = joblib.load(ENCODER_PATH)
    
    print(f"[*] Nạp Test Data: {TEST_DATA_PATH}...")
    ...
    preds_encoded = get_predictions(model, X_test_scaled, device)
    preds_labels = encoder.inverse_transform(preds_encoded)
    
    print("\n=========================================================")
    print("BÁO CÁO KẾT QUẢ V7 (CHÍNH THỨC DÙNG CHO LUẬN VĂN)")
    print("=========================================================")
    
    acc = accuracy_score(y_test, preds_labels)
    b_acc = balanced_accuracy_score(y_test, preds_labels)
    mcc = matthews_corrcoef(y_test, preds_labels)
    ...
    report = classification_report(y_test, preds_labels, target_names=encoder.classes_, digits=4)
    print(report)
    
    print("--- Confusion Matrix Tuyệt Đối ---")
    labels = encoder.classes_
    cm = confusion_matrix(y_test, preds_labels, labels=labels)
