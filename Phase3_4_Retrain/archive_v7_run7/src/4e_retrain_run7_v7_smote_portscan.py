...
def main():
    print("="*60)
    print("RETRAIN V7.1 (SMOTE EXCLUSIVE PORTSCAN + FOCAL LOSS)")
    print("="*60)
    ...
    print("\n[*] Áp dụng SMOTE (Chỉ cho PortScan)...")
    ...
    for cls_idx in unique:
        if cls_idx == portscan_idx:
            sampling_strategy[cls_idx] = 15000
        else:
            sampling_strategy[cls_idx] = class_counts[cls_idx]
            
    smote = SMOTE(sampling_strategy=sampling_strategy, random_state=42)
    X_train_smote, y_train_smote = smote.fit_resample(X_train, y_train)
    
    print("--- Phân phối sau SMOTE ---")
    u2, c2 = np.unique(y_train_smote, return_counts=True)
    ...
    scaler.fit_custom_scaler(X_train_smote[:, 77:])
    ...
    print("\n[*] Cấu hình Focal Loss (Không dùng explicit class weights)...")
    ...
    y_tensor = torch.LongTensor(y_train_smote).to(device)
