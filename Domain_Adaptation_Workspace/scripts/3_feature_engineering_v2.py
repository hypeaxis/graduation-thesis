...
# ----------------- FEATURE MAPPING FUNCTIONS (WITH NEW FEATURES) -----------------
...
def load_and_preprocess_cic2017(filepath, sample_size=30000):
    ...
    # Tái tạo cột cho df cic-ids vì nó không chứa các Custom feature mới này
    df = extract_custom_features(df)
    df.replace([np.inf, -np.inf], 0, inplace=True)
    df.fillna(0, inplace=True)
...
def main():
    print("="*60)
    print("GIAI ĐOẠN 3: FEATURE ENGINEERING (MODEL SURGERY)")
    print("="*60)
    ...
    print("[*] Nạp Testbed Data (có thêm 3 Feature mới)...")
    ...
    # --- SCALER EXPANSION ---
    print("[*] Áp dụng Scaler mở rộng...")
    scaler_old = joblib.load(os.path.join(MODEL_DIR, 'scaler_stage1.pkl'))
    
    X_mixed_old_features = X_mixed_train[:, :77]
    X_mixed_new_features = X_mixed_train[:, 77:]
    
    X_tb_val_old_features = X_tb_val[:, :77]
    X_tb_val_new_features = X_tb_val[:, 77:]
    
    X_mixed_scaled_old = scaler_old.transform(X_mixed_old_features)
    X_tb_val_scaled_old = scaler_old.transform(X_tb_val_old_features)
    
    scaler_new = StandardScaler()
    X_mixed_scaled_new = scaler_new.fit_transform(X_mixed_new_features)
    X_tb_val_scaled_new = scaler_new.transform(X_tb_val_new_features)
    
    X_mixed_scaled = np.hstack((X_mixed_scaled_old, X_mixed_scaled_new))
    X_tb_val_scaled = np.hstack((X_tb_val_scaled_old, X_tb_val_scaled_new))
    ...
    print("[*] Khởi tạo mô hình 80 Features và chép đè trọng số (Model Surgery)...")
    ...
    # Chép đè trọng số
    # 1. Feature Embedding cho 77 cột đầu
    for i in range(77):
        model.feature_embedding.feature_embeddings[i].load_state_dict(model_old.feature_embedding.feature_embeddings[i].state_dict())
    model.feature_embedding.cls_token.data.copy_(model_old.feature_embedding.cls_token.data)
    
    # 2. Transformer Blocks và Classifier
    ...
    # --- LAYER FREEZING (Bảo vệ trí nhớ) ---
    print("[*] Đóng băng (Freeze) 77 Embedding gốc và 2 khối Attention đầu...")
    for i in range(77):
    ...
    print("[*] Bắt đầu Fine-Tuning với 80 Features...")
    best_b_acc = -1
    best_model_path = '../models/finetuned_stage3_feature_eng.pt'
    ...
    print("\n[*] Đánh giá Mô hình tốt nhất (Ablation Study)...")
    ...
    X_cic_old = X_cic_test[:, :77]
    X_cic_new = X_cic_test[:, 77:]
    X_cic_scaled_old = scaler_old.transform(X_cic_old)
    X_cic_scaled_new = scaler_new.transform(X_cic_new)
    X_cic_test_scaled = np.hstack((X_cic_scaled_old, X_cic_scaled_new))
    ...
    log_data = {
        ...
        "config": {
            "strategy": "Phase 3 (+3 Custom Features, Model Surgery)",
            "learning_rate": 1e-5,
            "oversampler": "RandomOverSampler",
            "scaler": "Hybrid (Old + New)"
        },
        "metrics_testbed": final_tb_metrics,
        "metrics_cic_2017": cic_test_metrics
    }
    
    with open('../results/exp3_feature_engineering.json', 'w') as f:
        json.dump(log_data, f, indent=4)
    print("\n[*] Đã lưu kết quả vào results/exp3_feature_engineering.json")
